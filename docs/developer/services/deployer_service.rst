...
JWT issuer provided to the deployer at service initialization. If the
:ref:`resource_manager` is configured, the deployer exchanges the bearer token
for a resource manager-signed token that is used to ascertain the user's access
to the requested resources. The token exchange happens on behalf of the
...
*DEPLOYER_CLIENT_SECRET*: Client credentials used for OIDC authentication.
(default ``None``)
...
*DEPLOYER_TOKEN_SCOPE_KEY*: Key inside JWT containing scopes. Use
``https://rm.datascience.ch/scope`` in combination with the resource manager.
(default ``None``)
...
authorization tokens. (default ``None``)

*SQLALCHEMY_DATABASE_URI*: The URI of the database to be used for preserving
internal state. (default ``sqlite:///deployer.db``)

*SQLALCHEMY_TRACK_MODIFICATIONS*:  Should Flask-SQLAlchemy track modifications
of objects. (default ``False``)

*WSGI_NUM_PROXIES*:  The number of proxy servers in front of the app. Disable
proxy fixer by setting value evaluating to ``False``. (default ``None``)
