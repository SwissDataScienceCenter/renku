...
3. For the launcher’s environment:
    1. If you want to **Create from Code**, please note that only Python environments work on CSCS. If you’re unfamiliar with creating a code-based environment on Renku, see [How to create an environment with custom packages installed](../sessions/guides/create-environment-with-custom-packages-installed)
