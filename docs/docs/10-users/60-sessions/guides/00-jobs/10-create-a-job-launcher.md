...
3. Choose and configure an **environment**:
   - **Create from code** — Renku builds an environment from a code repository (for example, one with an `environment.yml` or `requirements.txt`). See [How to create an environment with custom packages installed](../environments/create-environment-with-custom-packages-installed).
   - **Custom image** — use your own Docker image. See [How to use your own docker image for a Renku session](../environments/use-your-own-docker-image-for-renku-session).
