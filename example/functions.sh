...
function getAccessToken() {
    ...
    CMD="$CURL_CMD | $JQ_CMD"
    echo $CMD
    RENGA_ACCESS_TOKEN=$(eval $CMD)
}

function getFirstBucket() {
    echo "Listing buckets..."
    CURL_CMD="curl -s -X GET \"$RENGA_API_URL/api/storage/explore/list\" \
        -H \"accept: application/json\" \
        -H \"Authorization: Bearer $RENGA_ACCESS_TOKEN\""
    JQ_CMD="jq \".[0].id\""
    CMD="$CURL_CMD | $JQ_CMD"
    echo $CMD
    local __resultvar=$1
...
function createBucket() {
    ...
    CURL_CMD="curl -s -X POST \"$RENGA_API_URL/api/storage/authorize/create_bucket\" \
        -H \"accept: application/json\" \
...
function listFiles() {
    ...
    CURL_CMD="curl -s -X GET \"$RENGA_API_URL/api/storage/explore/$bucket/files\" \
        -H \"accept: application/json\" \
        -H \"Authorization: Bearer $RENGA_ACCESS_TOKEN\""
...
function authReadFile() {
  ...
  if [ -n "$RENGA_DEPLOYMENT_ID" ]; then
      deployment_id_curl="-H \"Renga-DEPLOYMENT-ID: $RENGA_DEPLOYMENT_ID\""
  fi
  CURL_CMD="curl -s -X POST \"$RENGA_API_URL/api/storage/authorize/read\" \
      -H \"accept: application/json\" \
      -H \"content-type: application/json\" \
      $deployment_id_curl \
      -H \"Authorization: Bearer $RENGA_ACCESS_TOKEN\" \
...
function createFile() {
    ...
    CURL_CMD="curl -s -X POST \"$RENGA_API_URL/api/storage/authorize/create_file\" \
        -H \"accept: application/json\" \
        -H \"content-type: application/json\" \
        $deployment_id_curl \
        -H \"Authorization: Bearer $RENGA_ACCESS_TOKEN\" \
...
function authWriteFile() {
    ...
    CURL_CMD="curl -s -X POST \"$RENGA_API_URL/api/storage/authorize/write\" \
        -H \"accept: application/json\" \
        -H \"content-type: application/json\" \
        $deployment_id_curl \
        -H \"Authorization: Bearer $RENGA_ACCESS_TOKEN\" \
...
function readFile() {
    local auth_token=$1
    echo "Reading data..."
    CURL_CMD="curl -s -X GET \"$RENGA_API_URL/api/storage/io/read\" \
...
function writeFile() {
    ...
    local data=$2
    echo "Writing data..."
    CURL_CMD="curl -s -X POST \"$RENGA_API_URL/api/storage/io/write\" \
        -H \"accept: application/json\" \
