...
function searchForDataset(name: string, shouldExist = true) {
  cy.visit("/search");
  cy.dataCy("list-card").should("be.visible");
  cy.dataCy("type-entity-dataset").should("be.visible").check();
  cy.get("input[placeholder='Search...']").should("be.visible").type(name).type("{enter}");
  if (shouldExist)
