...
import ch.renku.acceptancetests.model.projects.{ProjectDetails, Public}
...
trait ExistingProjectSpecData {
  ...
  protected lazy val existingProjectDetails: Option[ProjectDetails] = {
    Option(getProperty("extant")) orElse sys.env.get("RENKU_TEST_EXTANT_PROJECT") match {
      case Some(s) => Some(ProjectDetails(Refined.unsafeApply(s), Public, Refined.unsafeApply("unused"), s))
      case None    => None
    }
  }
}
