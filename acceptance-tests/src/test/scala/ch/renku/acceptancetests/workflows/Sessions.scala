...
import ch.renku.acceptancetests.tooling.{AcceptanceSpec, AnonSessionConfig}
...
trait Sessions {
  ...
  def `launch a session`(implicit projectPage: ProjectPage): JupyterLabPage = {
    ...
    `start session & wait util it's not ready`
    docsScreenshots.takeScreenshot()
  ...
  def `launch anonymous session`(AnonSessionConfig: AnonSessionConfig): Option[JupyterLabPage] = {
    val projectId   = AnonSessionConfig.projectId
    ...
    click on projectPage.Sessions.tab
    if (AnonSessionConfig.isAvailable) {
  ...
  def `launch unprivileged session`(implicit AnonSessionConfig: AnonSessionConfig): JupyterLabPage = {
    val projectId = AnonSessionConfig.projectId
    implicit val projectPage: ProjectPage = ProjectPage(projectId)
    go to projectPage
    When(s"user goes to $projectId")
    And("clicks on the Sessions tab to launch an unprivileged notebook")
    click on projectPage.Sessions.tab
  ...
  private def `start session & wait util it's not ready`(implicit projectPage: ProjectPage): Unit = {
    projectPage.Sessions.verifyImageReady sleep (2 seconds)
