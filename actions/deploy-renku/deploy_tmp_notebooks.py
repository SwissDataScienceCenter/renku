...
def check_notebooks_chart_version(renku_chart_path=None):
    """Check the version of the renku-notebooks chart inside ./renku """

    chartdir = (
        renku_chart_path or Path(os.path.abspath(os.path.dirname(__file__))) / ".."
    )
    chart_dependencies = (
        subprocess.run(
    ...
    if renku_notebooks_version:
        return renku_notebooks_version
    raise RuntimeError("Unable to determine the renku-notebooks version.")


def deploy_tmp_notebooks(
    release_name,
    renku_namespace,
    ...
    """Do the anonymous sessions deployment."""
    kube_context = kube_context or k8s.config.list_kube_config_contexts()[1]["name"]
    ...
    k8s_api_client = k8s.client.CoreV1Api(
        k8s.client.ApiClient(
            k8s.config.load_kube_config(
                ...
                or k8s.config.list_kube_config_contexts()[1]["name"],
            )
        )
    )
    ...
    renku_values = get_values(release_name, kube_context, renku_namespace)
...
These values will override the ones automatically derived from primary Renku deployment. OPTIONAL.""",
    ...
    argparser.add_argument("--local-path", help="Path to local chart.", default=None)
    ...
    # Get the notebooks chart version
