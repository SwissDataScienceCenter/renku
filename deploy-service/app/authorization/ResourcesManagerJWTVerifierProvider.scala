 ...
 */
@Singleton
class ResourcesManagerJWTVerifierProvider @Inject() ( configuration: Configuration, wSClient: WSClient, executionContext: ExecutionContext ) {
  ...
  private[this] def getPublicKey: RSAPublicKey = {
    ...
    val futureKey = PublicKeyReader.getRSAPublicKey( configuration.getConfig( "resource-manager.key" ).get )
