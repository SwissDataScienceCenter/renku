...
//  private[this] implicit lazy val mapWrites: Writes[Map[P#Key, P]] = KeyFormat.mapWrites[P](implicitly[Writes[P]])
  private[this] implicit lazy val mapWrites: Writes[Map[P#Key, P]] = new Writes[Map[Key, P]] {
    def writes(m: Map[Key, P]): JsValue = implicitly[Writes[Iterable[P]]].writes(m.values)
  }
